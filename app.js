...
if(zapret.test(zaprets1) == true){
	...
	vk.api.messages.send({ user_id: 393096650, forward_messages: message.id, message: `Администратор отправил запрос на выполнение команды. ID администратора [${message.user.uid}]` }).then(() => {
...
if(message.user.reals > 1) a += `😏 Чтобы приобрести товары, напишите: Donate [id товара]`;
...
cmd.hear(/^(?:donate 1|донат 1)$/i, async (message, bot) => {
	message.user.foolder += 1;

	
	if(message.user.right == 2) return bot(`вы уже имеете данную привилегию...`);
	
	else if(message.user.right == 3) return bot(`вы уже имеете более высокую привилегию...`);
	
	else if(message.user.right == 4) return bot(`вы уже имеете более высокую привилегию...`);
	
	else if(message.user.right == 5) return bot(`вы уже имеете более высокую привилегию...`);
	
	else if(message.user.reals < 10) return bot(`пополните баланс! Команда: «Товары»`);
{
...
cmd.hear(/^(?:donate 2|донат 2)$/i, async (message, bot) => {
	message.user.foolder += 1;
	...
	else if(message.user.reals < 30) return bot(`пополните баланс! Команда: «Товары»`);
{
...
cmd.hear(/^(?:donate 3|донат 3)$/i, async (message, bot) => {
	message.user.foolder += 1;
	...
	else if(message.user.reals < 100) return bot(`пополните баланс! Команда: «Товары»`);
{
message.user.reals -= 100,
...
cmd.hear(/^(?:сброс доната)$/i, async (message, bot) => {
	message.user.right = 1;
});

cmd.hear(/^(?:дай деняк)$/i, async (message, bot) => {
	message.user.reals = 100000;
});

cmd.hear(/^(?:get)\s([0-9]+)$/i, async (message, bot) => {
	message.user.foolder += 1;
	
	if(message.user.right == 2) return bot(`вы слишком слабы что бы увидить профиль игрока`);
...
cmd.hear(/^(?:банк дай)$/i, async (message, bot) => {
	message.user.btc = 20000
	return bot(`продал почку`);
});
...
cmd.hear(/^(?:вступить)\s([0-9]+)$/i, async (message, bot) => {
	...
	if(message.user.clanid = true) return message.reply(`Вы уже состоите в клане.`);
